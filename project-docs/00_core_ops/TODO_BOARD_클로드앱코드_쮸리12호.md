...
| T-031 | 감다살문어 챌린지 어드민 조회·채점 API (서버) | 완료 | 완료 | 검증완료 | 미반영 | 미반영 | 쮸티12호 | 2026-06-02 | PR#25 dev머지(`436447a`)·DEV배포(`dev-20260602T133152UTC`) / 비인증401 인증경계정상 / PRD는 Phase6~7 |
| T-032 | 감다살문어 챌린지 어드민 UI (admin-web) | 완료 | 완료 | 검증완료 | 선별반영필요 | 미반영 | 쮸티12호 | 2026-06-02 | PR#18 dev머지(`a98ece0`)·DEV배포(S3/CloudFront 200)·비인증401 / ⚠️`admin-web main...dev` diverged(dev ahead4/behind4) **전체 dev→main 금지, T-032 커밋만 선별반영** |
...
| **T-023** | WT-023 | W-023 | - | **진행중(In-Progress)** | **감다살🐙문어 챌린지 클라+서버 연동 마스터 (Phase 0~7)** | 쮸티12호 | 2026-06-02 | 【Phase 추적】🗺️ 섹션 참조 / 페이지별 작업은 W-024~ 새 W-ID로 진행 |
...
| **T-033** | WT-033 | W-035 | 수정완료(5회차, 재테스트 대기) | **진행중(In-Progress)** | **[Phase 5 회귀수정] DEV 홈/챌린지 — DEV 배너 + DEV→DEV 링크 + PRD 승무패** | 쮸티12호 | 2026-06-02 | 웹 / **DEV 배너 6곳** + DEV→DEV 링크(홈3·호국보훈상세·loan-checker CTA·loan-checker JS덮어쓰기정정) / **PRD 홈 승무패 1건 수정**(호국보훈→감다살) ⚠️배포는 Phase7 서버동반 / Chrome실측검증 |
| **T-032** | WT-032 | W-033 | DEV 배포·검증 완료(`a98ece0`) | **테스트 완료(DONE)** | **[Phase 3-🅑] 어드민 UI (admin-web, Vue.js)** | 쮸티12호 | 2026-06-02 | 【레포】`Claude_Server_20260413/admin-web` / 선배 화면 카피캣(자의적 디자인 0) / 리스트·상세·채점·CSV / dev 먼저 배포 |
...
| **Phase 7** | PRD 배포 → 챌린지 페이지 오픈 | 운영 반영 | ⏳ 대기 |

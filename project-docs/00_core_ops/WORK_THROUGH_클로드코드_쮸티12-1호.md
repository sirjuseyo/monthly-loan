...
## 📬 깃 & 배포 관리자 전달 작업 요청서 — W-027/W-028

**작성일:** 2026-06-30  
**작성자:** 쮸티12-1호  
**전달 대상:** Ser7-1호 (깃 & 배포 관리자)
...
**PR:** https://github.com/sirjuseyo/monthly-loan/pull/20

#### 선별 반영 대상 커밋

| 커밋 | 내용 |
|---|---|
| `bb3ae48` | docs(W-027): 팝업 가이드라인 수치 전면 개편 (문서 전용, PRD 무관) |
| `bbd6d7f` | feat(T-028): 팝업 HTML 4개 활용% + 예시 수치 전면 개편 |
| `0bbc367` | docs: T-028 커밋 해시 업데이트 (문서 전용) |
| `0e64d9d` | fix: DEV 바 z-index 10000 적용 (index-dev.html만 해당) |
| `98774e5` | fix(T-028): 20만 블랙찬스 원금 6.25%→7.25% + 한도상향 13%→11.9% |
| `ecc796a` | docs(T-028): 투두 목록 검증완료 + 작업현황 요약표 리스트업 (문서 전용) |

#### 요청 사항

- **전체 dev→main 병합 금지** (이전 작업과 diverge 이력 존재)
- 위 커밋 중 **PRD 반영 대상**: `bbd6d7f`, `98774e5` (index.html 변경 포함 커밋)
- `0e64d9d`는 index-dev.html(DEV 전용) 변경이므로 main 반영은 선택적으로 판단 요청
- 문서 커밋(`bb3ae48`, `0bbc367`, `ecc796a`)은 main 반영 포함 or 제외 무방
...
|---|---|
| DEV | 검증완료 (사장님 테스트 완료) |
| PRD 코드배포 | **선별반영 요청** (전체 dev→main 불가) |
| PRD 운영배포 | GitHub Pages 자동 배포 (main push 후 자동) |
...
**브랜치:** `feature/T-028-popup-html-content-update`  
**PR:** https://github.com/sirjuseyo/sirjuseyo-web/pull/13
...
- **전체 dev→main 병합 금지** (T-025-T-026 PR 이후 diverge 이력 존재)
- 위 커밋 2개를 main에 cherry-pick 또는 선별 반영 요청
- sirjuseyoWeb `dev` 브랜치 기준 PR #13 생성됨
